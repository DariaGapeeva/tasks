		...
		<table className={styles.table}>
			...
			{props.users.map(user =>
				<tr key={user.id}>
					...
					<td>{user.name} <input disabled={props.inputDisabled} value={user.id === props.currentUser ? props.name : ''} onChange={(event) => props.updateName(event.target.value, user.id)}></input></td>
					<td>{user.username} <input disabled={props.inputDisabled} value={user.id === props.currentUser ? props.username : ''} onChange={(event) => props.updateUserName(event.target.value, user.id)} ></input></td>
